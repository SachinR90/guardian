...
///
/// NavigationDelegate is used as the delegate of a NavigationCoordinator's rootViewController
/// to allow for push-transitions to specify animations.
///
/// NavigationDelegate conforms to the `UINavigationControllerDelegate` protocol
...
///
open class NavigationDelegate: NSObject {
  ...
  var popCompletion: ((UIViewController) -> Void)?
...
extension NavigationDelegate: UINavigationControllerDelegate {
  ...
  open func navigationController(
    ...
    {
      popCompletion?(poppedViewController)
    }
  ...
  public func navigationControllerSupportedInterfaceOrientations(_: UINavigationController) -> UIInterfaceOrientationMask {
    .portrait
  }
}
  ...
  open func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
    switch gestureRecognizer {
      case navigationController?.interactivePopGestureRecognizer:
        let delegateAction = NavigationDelegate.interactivePopGestureRecognizerDelegateAction

        guard let delegate = interactivePopGestureRecognizerDelegate,
              delegate.responds(to: delegateAction)
        else {
...
extension UINavigationController {
  var animationDelegate: NavigationDelegate? {
    delegate as? NavigationDelegate
  }
}
